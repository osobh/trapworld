...
function Graph() {
  ...
  this.addEdge = function(top, bottom, left, right, weight) {
    ...
    var west = this.findNode(left);
    var east = this.findNode(right);
    // this.edges.push(new GraphEdge(north, weight))
    // this.edges.push(new GraphEdge(south, weight))
    // this.edges.push(new GraphEdge(west, weight))
    // this.edges.push(new GraphEdge(east, weight));
  };
  ...
  function findNodeToPlaceCrimeWeight(x, y, weight, squares){ //crime has lat and long & serverity in object form
    for (var i = 0; i < squares.length; i++){
      // console.log(x, y)
...
trapGraph.getData('https://data.sfgov.org/resource/9v2m-8wqu.json');
